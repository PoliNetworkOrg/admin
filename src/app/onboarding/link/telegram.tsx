...
export function TelegramLink({ botUsername }: { botUsername: string }) {
  ...
  const { data: savedLink, update, remove } = useLocalStorage<SavedLink>("linktg")
  const timeLeft = savedLink ? savedLink.ttl - (Date.now() - savedLink.startTime) / 1000 : null
  ...
  if (savedLink === undefined) return null
  if (success)
    return (
  ...
  return savedLink && timeLeft ? (
    <div className="flex flex-col items-center gap-4">
      ...
      <p className="text-muted-foreground pt-2 text-xs">
        ...
        <button type="button" className="cursor-pointer underline" onClick={reset}>
          Click to reset
        </button>
      </p>
    </div>
  ) : (
    <form onSubmit={handleSubmit} className="mx-auto flex w-full max-w-80 flex-col items-center gap-6">
...
function Timer({ ttl, timeLeft: pTimeLeft, onEnd }: { ttl: number; timeLeft: number; onEnd: () => void }) {
