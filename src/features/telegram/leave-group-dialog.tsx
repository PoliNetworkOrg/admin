...
export function LeaveGroupDialog({ chatId, title }: { chatId: number; title: string }) {
  ...
  async function leaveGroup() {
    ...
    try {
      ...
      try {
        ...
        toast.warning("The group was left, but the group list could not be refreshed.")
      }
    } catch (error) {
      console.error(error)
  ...
  return (
    <AlertDialog open={open} onOpenChange={(nextOpen) => !pending && setOpen(nextOpen)}>
      <AlertDialogTrigger render={<Button variant="destructive" size="icon-sm" title="Leave" />}>
        <Trash2 />
